...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($x = 1; $x <= 4; $x++) {
            $arr = ['name' => fake()->name(), 'address' => fake()->address(), 'phone' => fake()->phoneNumber(), 'managers_id' => fake()->numberBetween(
                $min = 1,
                $max = 10
            ), 'email' => fake()->unique()->safeEmail(), 'open_date' => fake()->date(), 'status' => fake()->text(10),];
